...
# Chunks illustrate the workflow; they are not evaluated at build time so the
# vignette renders without the full modelling dependencies and example data.
...
These come from the bundled `bumpus` sparrow data (the full workflow is in
Section 5):

```r
prepared <- prepare_selection_data(bumpus, "survival", c("weight", "total_length"))

uni <- univariate_spline(prepared, "survival", "weight", fitness_type = "binary",
                         bootstrap = TRUE)
plot_univariate_fitness(uni, "weight", classic_plot = TRUE)
```

```{r univariate-fig, echo=FALSE, eval=TRUE, out.width="70%", fig.cap="Univariate fitness function: a cubic spline with a bootstrapped 95% interval."}
knitr::include_graphics("figures/univariate_fitness_weight.png")
```

```{r surface-fig, echo=FALSE, eval=TRUE, out.width="70%", fig.cap="Correlated fitness surface over two standardised traits."}
knitr::include_graphics("figures/correlated_surface.png")
```

```{r landscape-fig, echo=FALSE, eval=TRUE, out.width="70%", fig.cap="Adaptive landscape: mean fitness across population mean phenotypes."}
knitr::include_graphics("figures/adaptive_landscape.png")
```
...
```{r, message=FALSE, warning=FALSE}
# All modelling, plotting, and helper functions ship with the installed package.
...
selection_differential(
  ...
  fitness_col,
  trait_col,
  standardized = TRUE, # Whether trait is already standardized
...
**Technical details:**
- Penalised cubic regression spline (`bs = "cr"`) with the smoothing parameter chosen by generalised cross-validation, following Schluter (1988)
- For binary fitness: binomial GAM with logit link; for continuous: Gaussian GAM on relative fitness
...
correlated_fitness_surface(
  ...
  scale_traits = TRUE,
  k = NULL # Basis dimension for GAM method; chosen from the data if NULL
)
```
...
**Purpose:** combine selection differentials and gradients into a single, publication-ready table in one call.
